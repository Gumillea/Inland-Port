...
public class IPBoatItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (hitresult.getType() == HitResult.Type.MISS) {
            ...
            List<Entity> list = level.getEntities(player, player.getBoundingBox().expandTowards(vec3.scale(5.0F)).inflate((double)1.0F), EntitySelector.NO_SPECTATORS.and(Entity::isPickable));
            if (!list.isEmpty()) {
    ...
    private Boat getBoat(Level level, HitResult result, ItemStack stack, Player player) {
        Vec3 vec3 = result.getLocation();
        EntityType<?> entityType = BuiltInRegistries.ENTITY_TYPE.get(this.type);
        Entity entity = entityType.create(level);
        if (entity instanceof IPBoat boat) {
