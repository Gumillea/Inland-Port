...
public class IPAttributeModifier {
    ...
    private static void addAttributeModifier() {
        if (InlandPortConfig.Startup.ENABLE_HEALING_EFFICIENCY.get()) {
        ...
        if (InlandPortConfig.Startup.ENABLE_DEBUFF_RESISTANCE.get()) {
            ...
            AttrUtil.add(MobEffects.WITHER, IPAttributes.DEBUFF_RESISTANCE, InlandPort.MODID, -0.1, AttributeModifier.Operation.ADD_MULTIPLIED_BASE);
        }

        if (InlandPortConfig.Startup.ENABLE_DODGE_CHANCE.get()) {
        ...
        if (InlandPortConfig.Startup.ENABLE_ITEM_USAGE_SPEED.get()){
